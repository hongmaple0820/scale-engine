...
```text
请作为 COO、Engineering Director 和 Agent Orchestrator，主导 SCALE Engine 的 Agentic 公司化协作流程。
...
```text
请作为 Technical Program Manager 和 Agent Runtime Lead，编排 SCALE Engine 的多 Agent 治理式交付。
...
```text
请作为 Quality Director 和 Red Team Lead，对 SCALE Engine 执行互审红队与自我修正闭环。
...
```text
请作为 Autonomous Delivery Lead 和 Cost Controller，推进 SCALE Engine 的长任务，但必须严格控制预算和闭环质量。

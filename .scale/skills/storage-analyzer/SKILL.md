...
## SCALE Workflow Boundary

When this vendored skill is used by SCALE Engine, scanning and report generation
are the default workflow. Do not run `scripts/server.py`, move files to Trash, or
hard-delete paths unless the user explicitly asks for cleanup after seeing the
report and confirms the exact action. Record evidence for `read-only-scan`,
`html-report`, and `cleanup-confirmation-boundary`.

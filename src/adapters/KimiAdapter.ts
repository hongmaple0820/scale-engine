// SCALE Engine — Kimi Adapter
// 生成 .kimi/settings.json + .kimi/rules.md + .kimi/skills/
// Kimi: Moonshot AI assistant (https://kimi.moonshot.cn)
...
export class KimiAdapter implements IAgentAdapter {
  readonly agentType = 'kimi'
  private projectDir: string = '.'
  private scaleDir: string = '.scale'

  getSettingsPath(): string {
    return join(this.projectDir, '.kimi', 'settings.json')
  }

  getKnowledgeDocPath(): string {
    return join(this.projectDir, '.kimi', 'rules.md')
  }

  getSkillsDir(): string {
    return join(this.projectDir, '.kimi', 'skills')
  }

  isInstalled(): boolean {
    return existsSync(join(this.projectDir, '.kimi'))
  }
...
`
  ...
  async init(config: AdapterConfig): Promise<InitResult> {
    ...
    // 2. Create .kimi/ directory + settings.json
    const kimiDir = join(this.projectDir, '.kimi')
    if (!existsSync(kimiDir)) {
      mkdirSync(kimiDir, { recursive: true })
      created.push(kimiDir)
    } else {
      skipped.push(kimiDir)
    }
    ...
    // 4. Create .kimi/rules.md (knowledge document)
    ...
    logger.info({ created: created.length, skipped: skipped.length }, 'SCALE init (kimi) completed')

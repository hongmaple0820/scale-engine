...
- Default third-party expectations for the full workflow are: rtk for shell execution, gbrain for memory, CodeGraph for code structure, Graphify for knowledge graph artifacts, and browser/E2E tooling when the task touches UI or web flows.
- Do not silently downgrade to a no-memory or no-knowledge workflow. If gbrain, CodeGraph, or Graphify is unavailable, mark it as blocked or degraded in the plan and include the exact setup command.

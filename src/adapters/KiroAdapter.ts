...
export class KiroAdapter implements IAgentAdapter {
  readonly agentType = 'kiro'
  private projectDir: string = '.'
  private scaleDir: string = '.scale'

  getSettingsPath(): string {
    return join(this.projectDir, '.kiro', 'settings.json')
  }

  getKnowledgeDocPath(): string {
    return join(this.projectDir, '.kiro', 'rules', 'SCALE.md')
  }

  getSkillsDir(): string {
    return join(this.projectDir, '.kiro', 'skills')
  }

  isInstalled(): boolean {
    return existsSync(join(this.projectDir, '.kiro'))
  }
...
`
  ...
  async init(config: AdapterConfig): Promise<InitResult> {
    ...
    // 2. Create .kiro/ directory + settings.json
    const kiroDir = join(this.projectDir, '.kiro')
    ...
    logger.info({ created: created.length, skipped: skipped.length }, 'SCALE init (kiro) completed')

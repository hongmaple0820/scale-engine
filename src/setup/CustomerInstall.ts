  ...
  try {
    ...
    if (options.skipDeps || dependencyPacks.length === 0) {
      ...
      setup = await runSetupWizard({
        ...
        interactive: false,
        lang,
        memoryProvider: options.memoryProvider ?? (dependencyPacks.includes('memory') || dependencyPacks.includes('full') ? 'gbrain' : undefined),
        memoryMode: options.memoryMode ?? (dependencyPacks.includes('memory') || dependencyPacks.includes('full') ? 'external-first' : undefined),

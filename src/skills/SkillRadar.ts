...
const DOMAIN_CONFIG: Record<string, {
  ...
  documentParsing: {
    keywords: ['pdf', 'docx', 'pptx', 'xlsx', 'ocr', 'document parsing', 'parse document', 'extract text', 'knowledge ingestion'],
    filePatterns: [/\.(pdf|docx|pptx|xlsx|csv|png|tiff)$/i],
    categories: ['document-parsing'],
    evidence: ['parse-output', 'source-citation'],
    fallback: 'Extract text manually with source citations and record parsing limitations.',
  },
  diagramming: {
    keywords: ['diagram', 'd2', 'architecture diagram', 'flow chart', 'sequence diagram', 'er diagram'],
    filePatterns: [/\.d2$/i, /(^|\/)docs\/(architecture|diagrams)\//i],
    categories: ['diagramming'],
    evidence: ['diagram-validate', 'architecture-review'],
    fallback: 'Use version-controlled Markdown diagrams and record manual validation notes.',
  },
  orchestration: {
    keywords: ['orchestration', 'metaskill', 'agent harness', 'model routing', 'token cost', 'opensquilla'],
    filePatterns: [/(^|\/)src\/(orchestrator|orchestration|workflow|runtime)\//i],
    categories: ['orchestration'],
    evidence: ['orchestration-review', 'cost-risk-note'],
    fallback: 'Review SCALE native orchestrator behavior and document why external harness ideas were skipped.',
  },
  planning: {
